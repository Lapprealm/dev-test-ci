...
class CallCenter
{
	...
	public function makeCall(
		...
		if (
			"__destruct" === $methodName ||
			0 == count($prophecy->getMethodProphecies())
		) {
		...
		if (!count($matches)) {
			throw $this->createUnexpectedCallException(
				$prophecy,
				$methodName,
				$arguments
			);
	...
	{
		return array_values(
			array_filter($this->recordedCalls, function (Call $call) use (
				$methodName,
				$wildcard
			) {
