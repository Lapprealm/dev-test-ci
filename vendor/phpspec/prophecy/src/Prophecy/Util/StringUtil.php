...
class StringUtil
{
	...
	public function stringify($value, $exportObject = true)
	{
		if (is_array($value)) {
			if (range(0, count($value) - 1) === array_keys($value)) {
				return "[" .
					implode(", ", array_map([$this, __FUNCTION__], $value)) .
					"]";
			}
		...
		if (is_object($value)) {
			return $exportObject
				? ExportUtil::export($value)
				: sprintf("%s:%s", get_class($value), spl_object_hash($value));
		}
	...
	public function stringifyCalls(array $calls)
	{
		...
		return implode(
			PHP_EOL,
			array_map(function (Call $call) use ($self) {
				return sprintf(
					"  - %s(%s) @ %s",
					$call->getMethodName(),
					implode(
						", ",
						array_map([$self, "stringify"], $call->getArguments())
					),
					str_replace(
