...
class ArrayEntryTokenSpec extends ObjectBehavior
{
	...
	function it_wraps_non_token_key_into_ExactValueToken($object, $value)
	{
		...
		$this->getKey()->shouldHaveType(
			"\Prophecy\Argument\Token\ExactValueToken"
		);
	}

	function it_scores_array_half_of_combined_scores_from_key_and_value_tokens(
		$key,
		$value
	) {
	...
	function it_does_not_score_array_if_key_and_value_tokens_do_not_score_same_entry(
		$key,
		$value
	) {
