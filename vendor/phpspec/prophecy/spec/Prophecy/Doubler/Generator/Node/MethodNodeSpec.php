...
class MethodNodeSpec extends ObjectBehavior
{
	...
	{
		$this->shouldThrow("InvalidArgumentException")->duringSetVisibility(
			"stealth"
		);
	}
	 ...
	 */
	function its_useParentCode_causes_method_to_call_parent(
		$argument1,
		$argument2
	) {
	...
	function its_reference_returning_methods_will_generate_exceptions()
	{
		...
		$this->setReturnsReference();
		$this->getCode()->shouldReturn(
			"throw new \Prophecy\Exception\Doubler\ReturnByReferenceException('Returning by reference not supported', get_class(\$this), 'getTitle');"
		);
	}
