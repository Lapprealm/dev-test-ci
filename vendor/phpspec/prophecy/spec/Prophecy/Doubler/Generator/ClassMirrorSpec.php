...
class ClassMirrorSpec extends ObjectBehavior
{
	...
	function it_reflects_a_class_by_mirroring_all_its_public_methods(
		...
		$class
			...
			->willReturn([$method1, $method2, $method3]);
		...
		$classNode->shouldBeAnInstanceOf(
	...
	function it_marks_passed_by_reference_args_as_passed_by_reference(
		...
		$method->isStatic()->willReturn(false);
		$method->returnsReference()->willReturn(false);
		$method->getParameters()->willReturn([$param1, $param2]);
	 ...
	 */
	function it_throws_an_exception_if_interface_provided_instead_of_class(
		$interface
	) {
	 ...
	 * @param ReflectionMethod $method
	 */
	function it_does_not_throw_exception_for_virtually_private_finals(
		$class,
		$method
	) {
		$class->getName()->willReturn("SomeClass");
	...
	function it_throws_an_exception_if_class_provided_in_interfaces_list($class)
	{
		...
		$this->shouldThrow("InvalidArgumentException")->duringReflect(null, [
			$class,
		]);
	}
	...
	{
		$classNode = $this->reflect(
			new ReflectionClass("ReflectionMethod"),
			[]
		);
