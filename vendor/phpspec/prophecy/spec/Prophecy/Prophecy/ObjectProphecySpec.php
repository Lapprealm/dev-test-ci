...
class ObjectProphecySpec extends ObjectBehavior
{
	...
	function it_sets_constructor_arguments_during_willBeConstructedWith_call(
		$lazyDouble
	) {
	 ...
	 */
	function it_should_proxy_makeProphecyMethodCall_to_CallCenter(
		$lazyDouble,
		$callCenter
	) {
		...
		$callCenter
			->makeCall($this->getWrappedObject(), "setName", ["everzet"])
			->willReturn(42);
	...
	function it_should_reveal_arguments_and_return_values_from_callCenter(
		...
		$callCenter
			->makeCall($this->getWrappedObject(), "setName", ["life"])
			->willReturn("answer");

		$this->makeProphecyMethodCall("setName", ["question"])->shouldReturn(
			42
		);
	}
	...
	function it_should_proxy_getProphecyMethodCalls_to_CallCenter(
		...
		$this->findProphecyMethodCalls("setName", $wildcard)->shouldReturn([
			$call,
		]);
	}
	...
	function its_addMethodProphecy_handles_prophecies_with_different_arguments(
		...
		$this->getMethodProphecies()->shouldReturn([
			"getUsername" => [$methodProphecy1, $methodProphecy2],
		]);
	}
	...
	function its_addMethodProphecy_handles_prophecies_for_different_methods(
		...
		$this->getMethodProphecies()->shouldReturn([
			"getUsername" => [$methodProphecy1],
			"isUsername" => [$methodProphecy2],
		]);
	}
	...
	function its_addMethodProphecy_throws_exception_when_method_has_no_ArgumentsWildcard(
		...
		)->duringAddMethodProphecy($methodProphecy);
	}
	...
	function it_returns_same_MethodProphecy_for_same_registered_signature(
		...
		$this->addMethodProphecy(
			$methodProphecy1 = $this->getProphecy(1, 2, 3)
		);
	...
	function it_returns_new_MethodProphecy_for_different_signatures(
		...
		$this->addMethodProphecy(
			$methodProphecy1 = $this->getProphecy(1, 2, 3, $value)
		);
	...
	function it_returns_new_MethodProphecy_for_all_callback_signatures(
		...
		$this->addMethodProphecy(
			$methodProphecy1 = $this->getProphecy(function () {})
		);
