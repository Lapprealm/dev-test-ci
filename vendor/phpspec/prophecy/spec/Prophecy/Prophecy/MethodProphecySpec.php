...
class MethodProphecySpec extends ObjectBehavior
{
	...
	function its_constructor_throws_MethodNotFoundException_for_unexisting_method(
		...
		)->during("__construct", [$objectProphecy, "getUnexisting", null]);
	}
	...
	function its_constructor_throws_MethodProphecyException_for_final_methods(
		...
		)->during("__construct", [$objectProphecy, "finalMethod", null]);
	}
	...
	function its_constructor_does_not_touch_third_argument_if_it_is_null(
		$objectProphecy
	) {
		$this->beConstructedWith($objectProphecy, "getName", null);
	...
	function it_adds_ReturnPromise_during_willReturn_call($objectProphecy)
	{
		...
		$this->willReturn(42);
		$this->getPromise()->shouldBeAnInstanceOf(
	...
	function it_adds_ReturnArgumentPromise_during_willReturnArgument_call_with_index_argument(
		...
		$promise->shouldBeAnInstanceOf(
			"Prophecy\Promise\ReturnArgumentPromise"
		);
		$promise
			->execute(["one", "two"], $objectProphecy, $this)
			->shouldReturn("two");
	}
	...
	function it_adds_CallbackPrediction_during_should_call_with_callback_argument(
		...
		$this->getPrediction()->shouldBeAnInstanceOf(
	...
	function it_checks_prediction_via_shouldHave_method_call(
		...
		$prediction
			->check(
				[$call1, $call2],
				$objectProphecy->getWrappedObject(),
				$this
			)
			->shouldBeCalled();
		$objectProphecy
			->findProphecyMethodCalls("getName", $arguments)
			->willReturn([$call1, $call2]);
	...
	function it_records_checked_predictions(
		...
		$this->getCheckedPredictions()->shouldReturn([
			$prediction1,
			$prediction2,
		]);
	}
