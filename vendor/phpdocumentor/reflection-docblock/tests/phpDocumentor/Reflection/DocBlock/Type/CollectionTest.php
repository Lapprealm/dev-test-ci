...
class CollectionTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testConstructWithNamespace()
	{
		...
		$this->assertEquals(
			"My\Space",
			$collection->getContext()->getNamespace()
		);
	...
	public function testAdd($fixture, $expected)
	{
		$collection = new Collection(
			[],
			new Context("\My\Space", ["Alias" => "\My\Space\Aliasing"])
		);
	...
	public function testAddWithoutNamespace($fixture, $expected)
	{
		$collection = new Collection(
			[],
			new Context(null, ["Alias" => "\My\Space\Aliasing"])
		);
		$collection->add($fixture);
	...
	public function testAddMethodsAndProperties($fixture, $expected)
	{
		$collection = new Collection(
			[],
			new Context(null, [
				"LinkDescriptor" => "\phpDocumentor\LinkDescriptor",
			])
		);
		$collection->add($fixture);
	...
	public function provideTypesToExpand($method, $namespace = "\My\Space\\")
	{
		return [
			...
			["DocBlock|Tag", [$namespace . "DocBlock", $namespace . "Tag"]],
			["DocBlock|null", [$namespace . "DocBlock", "null"]],
			[
				"\My\Space\DocBlock|Tag",
				["\My\Space\DocBlock", $namespace . "Tag"],
			],
			["DocBlock[]|null", [$namespace . "DocBlock[]", "null"]],
			["DocBlock[]|int[]", [$namespace . "DocBlock[]", "int[]"]],
			[
				"LinkDescriptor::setLink()",
				[$namespace . "LinkDescriptor::setLink()"],
			],
			[
				"Alias\LinkDescriptor::setLink()",
				["\My\Space\Aliasing\LinkDescriptor::setLink()"],
			],
		];
	}
	...
	public function provideTypesToExpandWithPropertyOrMethod($method)
	{
		return [
			...
			['LinkDescriptor::$link', ['\phpDocumentor\LinkDescriptor::$link']],
			[
