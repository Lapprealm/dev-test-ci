...
EOF;
	...
	{
		$files = $this->parser->parse(
			file_get_contents($this->path . "/index.yml")
		);
...
EOF;
	...
	public function testObjectSupportEnabled()
	{
		...
		$this->assertEquals(
			'{ foo: !php/object \'O:30:"Symfony\Component\Yaml\Tests\A":1:{s:1:"a";s:3:"foo";}\', bar: 1 }',
			$dump,
	...
	public function testBinaryDataIsDumpedBase64Encoded()
	{
		...
		$this->assertSame(
			$expected,
			$this->dumper->dump(["data" => $binaryData])
		);
	}
...
YAML;
	...
	public function testDumpEmptyStdClassInstanceAsMap()
	{
		$this->assertSame(
			"{  }",
			$this->dumper->dump(new \stdClass(), 2, 0, Yaml::DUMP_OBJECT_AS_MAP)
		);
	}
...
YAML;
	...
	public function testDumpingInlinedMultiLineIfRnBreakLineInTaggedValue()
	{
		...
		$this->assertSame(
			file_get_contents(
				__DIR__ .
					"/Fixtures/multiple_lines_as_literal_block_for_tagged_values.yml"
			),
			$this->dumper->dump(
				$data,
				2,
	...
	public function testDumpMultiLineStringAsScalarBlock()
	{
		...
		$this->assertSame(
			file_get_contents(
				__DIR__ . "/Fixtures/multiple_lines_as_literal_block.yml"
			),
			$this->dumper->dump(
				$data,
				2,
	...
	public function testDumpMultiLineStringAsScalarBlockWhenFirstLineHasLeadingSpace()
	{
		...
		$this->assertSame(
			file_get_contents(
				__DIR__ .
					"/Fixtures/multiple_lines_as_literal_block_leading_space_in_first_line.yml"
			),
			$this->dumper->dump(
				$data,
				2,
