...
class ParserTest extends TestCase
{
	...
	public function testSpecifications($expected, $yaml, $comment, $deprecated)
	{
		...
		if ($deprecated) {
			...
			$this->assertStringContainsString(
				true !== $deprecated
					? $deprecated
					: "Using the comma as a group separator for floats is deprecated since Symfony 3.2 and will be removed in 4.0 on line 1.",
				$deprecations[0]
			);
		}
	}
	...
	public function testNonStringMappingKeys($expected, $yaml, $comment)
	{
		$this->assertSame(
			$expected,
			var_export(
				$this->parser->parse($yaml, Yaml::PARSE_KEYS_AS_STRINGS),
				true
			),
			$comment
		);
...
EOF;
		...
		$tests[
			"Literal block chomping clip with embedded blank line inside unindented collection"
		] = [$expected, $yaml];
...
EOF;
		$this->assertEquals(
			["foo" => new B(), "bar" => 1],
			$this->parser->parse($input, Yaml::PARSE_OBJECT),
...
EOF;
	...
	public function testObjectSupportEnabledWithDeprecatedTag($yaml)
	{
		$this->assertEquals(
			["foo" => new B(), "bar" => 1],
			$this->parser->parse($yaml, Yaml::PARSE_OBJECT),
			"->parse() is able to parse objects"
		);
	}
...
YAML
	...
	public function testObjectForMapEnabledWithMappingUsingBooleanToggles(
		...
		$this->assertEquals(
			$expected,
			$this->parser->parse($yaml, false, false, true)
		);
	}
...
YAML;
	 ...
	 * @dataProvider invalidDumpedObjectProvider
	 */
	public function testObjectsSupportDisabledWithExceptionsUsingBooleanToggles(
		$yaml
	) {
		$this->expectException(
			"Symfony\Component\Yaml\Exception\ParseException"
...
EOT;
		$expected = [
			"test" =>
				'You can have things that don\'t look like strings here true yes you can',
		];
...
EOD;
	 ...
	 */
	public function testParseExceptionOnDuplicate(
		$input,
		$duplicateKey,
		$lineNumber
	) {
...
EOT;
	 ...
	 */
	public function testParserThrowsExceptionWithCorrectLineNumber(
		$lineNumber,
		$yaml
	) {
		$this->expectException(
		...
		$this->expectExceptionMessage(
			sprintf(
				'Unexpected characters near "," at line %d (near "bar: "123",").',
				$lineNumber
			)
		);
...
YAML
	...
	public function testExceptionWhenUsingUnsupportedBuiltInTags()
	{
		...
		$this->expectExceptionMessage(
			'The built-in tag "!!foo" is not implemented at line 1 (near "!!foo").'
		);
		$this->parser->parse("!!foo");
	}
...
INI;
	...
	{
		$longStringWithSpaces = str_repeat(
...
YAML;
		...
		$this->assertSame(
			...
			$this->parser->parse(
				$yaml,
				Yaml::PARSE_CONSTANT | Yaml::PARSE_KEYS_AS_STRINGS
			)
		);
	}
...
YAML;
		...
		$this->assertEquals(
			$expected,
			$this->parser->parse($yaml, Yaml::PARSE_OBJECT_FOR_MAP)
		);
	}
	...
	{
		$this->assertIsArray(
			$this->parser->parseFile(__DIR__ . "/Fixtures/index.yml")
		);
	}
	...
	public function testParsingNonExistentFilesThrowsException()
	{
		...
		$this->expectExceptionMessageMatches(
			'#^File ".+/Fixtures/nonexistent.yml" does not exist\.$#'
		);
	...
	public function testParsingNotReadableFilesThrowsException()
	{
		...
		$this->expectExceptionMessageMatches(
			'#^File ".+/Fixtures/not_readable.yml" cannot be read\.$#'
		);
...
EOE;
	...
	public function testDetectCircularReferences($yaml)
	{
		...
		$this->expectExceptionMessage(
			"Circular reference [foo, bar, foo] detected"
		);
		$this->parser->parse($yaml, Yaml::PARSE_CUSTOM_TAGS);
	}
...
YAML;
		...
		$this->assertSame(
			["a" => ["b" => "row\nrow2\n"], "c" => "d"],
			$this->parser->parse($longDocument)
		);
	}
}
