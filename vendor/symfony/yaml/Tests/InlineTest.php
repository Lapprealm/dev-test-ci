...
class InlineTest extends TestCase
{
	...
	public function testDeprecatedConstantTag($yaml, $expectedValue)
	{
		$this->assertSame(
			$expectedValue,
			Inline::parse($yaml, Yaml::PARSE_CONSTANT)
		);
	}
	...
	public function testDump($yaml, $value, $parseFlags = 0)
	{
		...
		$this->assertSame(
			$value,
			Inline::parse(Inline::dump($value), $parseFlags),
			"check consistency"
		);
	}
	...
	public function testParseReferences($yaml, $expected)
	{
		$this->assertSame(
			$expected,
			Inline::parse($yaml, 0, ["var" => "var-value"])
		);
	}
	...
	public function testParseReferencesAsFifthArgument($yaml, $expected)
	{
		$this->assertSame(
			$expected,
			Inline::parse($yaml, false, false, false, ["var" => "var-value"])
		);
	}
	...
	public function testParseMapReferenceInSequenceAsFifthArgument()
	{
		...
		$this->assertSame(
			[$foo],
			Inline::parse("[*foo]", false, false, false, ["foo" => $foo])
		);
	}
	 ...
	 */
	public function testParseUnquotedScalarStartingWithReservedIndicator(
		$indicator
	) {
		$this->expectException(ParseException::class);
		$this->expectExceptionMessage(
			sprintf(
				'cannot start a plain scalar; you need to quote the scalar at line 1 (near "%sfoo ").',
				$indicator
			)
		);
	...
	public function getTestsForParseWithMapObjects()
	{
		return [
			...
			[
				"[foo, bar: { foo: bar }]",
				["foo", "1" => (object) ["bar" => (object) ["foo" => "bar"]]],
			],
			[
				'[foo, \'@foo.baz\', { \'%foo%\': \'foo is %foo%\', bar: \'%foo%\' }, true, \'@service_container\']',
				[
			...
			[
				"{ foo  : bar, bar : {}  }",
				(object) ["foo" => "bar", "bar" => new \stdClass()],
			],
			[
				"{ foo  : [], bar : {}  }",
			...
			[
				"[foo, [[{}, {}], {}]]",
				["foo", [[new \stdClass(), new \stdClass()], new \stdClass()]],
			],
			[
				"[foo, {bar: {}}]",
				["foo", "1" => (object) ["bar" => new \stdClass()]],
			],
		];
	}
	 ...
	 */
	public function testParseTimestampAsUnixTimestampByDefault(
		$yaml,
		$year,
		$month,
		$day,
	 ...
	 * @dataProvider getTimestampTests
	 */
	public function testParseTimestampAsDateTimeObject(
		$yaml,
		$year,
		$month,
		$day,
	 ...
	 * @dataProvider getTimestampTests
	 */
	public function testParseNestedTimestampListAsDateTimeObject(
		$yaml,
		$year,
		$month,
		$day,
		...
		$this->assertEquals(
			$expectedNested,
			Inline::parse($yamlNested, Yaml::PARSE_DATETIME)
		);
	}
	...
	public function testUnfinishedInlineMap()
	{
		...
		$this->expectExceptionMessage(
			'Unexpected end of line, expected one of ",}" at line 1 (near "{abc: \'def\'").'
		);
