...
class Parser
{
	...
	public function __construct()
	{
		if (\func_num_args() > 0) {
			@trigger_error(
				sprintf(
					'The constructor arguments $offset, $totalNumberOfLines, $skippedLineNumbers of %s are deprecated and will be removed in 4.0',
					self::class
				),
				\E_USER_DEPRECATED
			);
			...
			if (
				...
				if (
					...
					@trigger_error(
						$this->getDeprecationMessage(
							"Starting an unquoted string with a question mark followed by a space is deprecated since Symfony 3.3 and will throw \Symfony\Component\Yaml\Exception\ParseException in 4.0."
						),
						\E_USER_DEPRECATED
					);
				}
				...
				try {
					...
					if (
						isset($values["key"][0]) &&
						"!" === $values["key"][0] &&
						Yaml::PARSE_CONSTANT & $flags
					) {
						$evaluateKey = true;
					}

					$key = Inline::parseScalar(
						$values["key"],
						0,
						null,
						$i,
				...
				if (!\is_string($key) && !\is_int($key)) {
					$keyType = is_numeric($key)
						? "numeric key"
						: "non-string key";
					@trigger_error(
						$this->getDeprecationMessage(
							sprintf(
								"Implicit casting of %s to string is deprecated since Symfony 3.3 and will throw \Symfony\Component\Yaml\Exception\ParseException in 4.0. Quote your evaluable mapping keys instead.",
								$keyType
							)
						),
						\E_USER_DEPRECATED
					...
					if (
						...
						if (!\array_key_exists($refName, $this->refs)) {
							if (
								...
								throw new ParseException(
									sprintf(
										'Circular reference [%s, %s] detected for reference "%s".',
										implode(
											", ",
											\array_slice(
												$this->refsBeingParsed,
	...
	private function parseValue($value, $flags, $context)
	{
		...
		try {
			...
			while ($this->moveToNextLine()) {
				...
				if (
					null === $quotation &&
					0 === $this->getCurrentLineIndentation()
				) {
					$this->moveToPreviousLine();
				...
				if (
					"" !== $escapedLine &&
					substr($escapedLine, -1) === $quotation
				) {
					break;
				}
			}
