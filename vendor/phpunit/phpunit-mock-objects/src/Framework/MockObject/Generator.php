...
class PHPUnit_Framework_MockObject_Generator
{
	...
	public function getMock(
		...
		if ($mockClassName != "" && class_exists($mockClassName, false)) {
			...
			if (
				...
				throw new PHPUnit_Framework_MockObject_RuntimeException(
					sprintf('Class "%s" already exists.', $mockClassName)
				);
			}
		}
	...
	public function getMockForAbstractClass(
		...
		if (
			...
			foreach ($reflector->getMethods() as $method) {
				if (
					$method->isAbstract() &&
					!in_array($method->getName(), $methods)
				) {
	...
	public function getMockForTrait(
		...
		if (!trait_exists($traitName, $callAutoload)) {
			throw new PHPUnit_Framework_MockObject_RuntimeException(
				sprintf('Trait "%s" does not exist.', $traitName)
			);
		}
	...
	private function generateMock(
		...
		if (is_array($type)) {
			foreach ($type as $_type) {
				if (!interface_exists($_type, $callAutoload)) {
					throw new PHPUnit_Framework_MockObject_RuntimeException(
						sprintf('Interface "%s" does not exist.', $_type)
					);
				}
		...
		if (isset($class)) {
			...
			if (
				$isInterface &&
				$class->implementsInterface("Traversable") &&
	...
	private function generateClassName($type, $className, $prefix)
	{
		...
		if ($className == "") {
			do {
				$className =
					$prefix . $type . "_" . substr(md5(microtime()), 0, 8);
	...
	private function generateMockedMethodDefinition(
		...
		$template->setVar([
			...
			"arguments_count" => !empty($arguments_call)
				? count(explode(",", $arguments_call))
				: 0,
			"class_name" => $className,
	...
	private function isMethodNameBlacklisted($name)
	{
		if (
			PHP_MAJOR_VERSION < 7 &&
			isset($this->legacyBlacklistedMethodNames[$name])
		) {
			return true;
		}
		...
		if (
			PHP_MAJOR_VERSION >= 7 &&
			isset($this->blacklistedMethodNames[$name])
		) {
			return true;
		}
	...
	{
		return method_exists("ReflectionParameter", "isVariadic") &&
	...
	{
		return method_exists("ReflectionParameter", "hasType") &&
			$parameter->hasType();
	}
	...
	{
		return method_exists("ReflectionMethod", "hasReturnType") &&
