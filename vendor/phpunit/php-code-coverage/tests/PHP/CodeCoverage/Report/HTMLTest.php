...
class PHP_CodeCoverage_Report_HTMLTest extends PHP_CodeCoverage_TestCase
{
	...
	public static function setUpBeforeClass()
	{
		...
		self::$TEST_REPORT_PATH_SOURCE =
			TEST_FILES_PATH . "Report" . DIRECTORY_SEPARATOR . "HTML";
	}
	...
	public function testForFileWithIgnoredLines()
	{
		...
		$report = new PHP_CodeCoverage_Report_HTML();
		$report->process(
			$this->getCoverageForFileWithIgnoredLines(),
			self::$TEST_TMP_PATH
		);
	...
	public function testForClassWithAnonymousFunction()
	{
		...
		$report = new PHP_CodeCoverage_Report_HTML();
		$report->process(
			$this->getCoverageForClassWithAnonymousFunction(),
			self::$TEST_TMP_PATH
		);
	...
	protected function assertFilesEquals($expectedFilesPath, $actualFilesPath)
	{
		...
		$actualFilesIterator = new RegexIterator(
			new FilesystemIterator($actualFilesPath),
			"/.html/"
		);
