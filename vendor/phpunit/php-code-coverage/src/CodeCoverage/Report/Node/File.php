...
class PHP_CodeCoverage_Report_Node_File extends PHP_CodeCoverage_Report_Node
{
	...
	protected function calculateStatistics()
	{
		...
		for (
			$lineNumber = 1;
			$lineNumber <= $this->linesOfCode["loc"];
			$lineNumber++
		) {
	...
	protected function processClasses(PHP_Token_Stream $tokens)
	{
		...
		foreach ($classes as $className => $class) {
			...
			$this->startLines[$class["startLine"]] =
				&$this->classes[$className];
			...
			foreach ($class["methods"] as $methodName => $method) {
				$this->classes[$className]["methods"][
					$methodName
				] = $this->newMethod($methodName, $method, $link);

				$this->startLines[$method["startLine"]] =
					&$this->classes[$className]["methods"][$methodName];
				$this->endLines[$method["endLine"]] =
					&$this->classes[$className]["methods"][$methodName];
			}
		}
	}
	...
	protected function processFunctions(PHP_Token_Stream $tokens)
	{
		...
		foreach ($functions as $functionName => $function) {
			...
			$this->startLines[$function["startLine"]] =
				&$this->functions[$functionName];
			$this->endLines[$function["endLine"]] =
				&$this->functions[$functionName];
		}
	}
