					...
					G.each(function (b) {
						...
						if (
							...
							J.forEach(function (a, c) {
								a.nonStackable
									? ((b[c].nonStackableSeries = I++), (J[c] = b[c]))
									: c > 0 &&
									  J[c - 1].nonStackable &&
									  J[c].values.map(function (a, b) {
						...
						m
							...
							(h = h || m),
							(i = i || n);
						var L = p.selectAll("g.nv-wrap.nv-multibar").data([b]),
							M = L.enter()
								.append("g")
						...
						O.append("g").attr("class", "nv-groups"),
							...
							N.append("clipPath")
								...
								.append("rect"),
							L.select("#nv-edge-clip-" + o + " rect")
								.attr("width", G)
								.attr("height", H),
							P.attr("clip-path", u ? "url(#nv-edge-clip-" + o + ")" : "");
						var Q = L.select(".nv-groups")
							.selectAll(".nv-group")
							.data(
								function (a) {
									return a;
						...
						R.delay &&
							...
							Q.style("stroke-opacity", 1).style("fill-opacity", 0.75);
						...
						var T = S.enter()
							...
							.attr("transform", function (a, c, d) {
								var e =
									...
									f = i(v && !b[d].nonStackable ? a.y0 : 0) || 0;
							...
							s(b[0].values[0], 0) &&
								...
								S.select("polyline")
									...
									.attr("points", function (a, c) {
										var d = s(a, c),
											e = (0.8 * m.rangeBand()) / (2 * (v ? 1 : b.length));
									...
									.attr("transform", function (a, c) {
										var d = m.rangeBand() / (2 * (v ? 1 : b.length)),
											e = r(a, c) < 0 ? n(r(a, c)) - n(0) : 0;
						...
						var U = S.watchTransition(E, "multibar", Math.min(250, B)).delay(
							function (a, c) {
								return (c * B) / b[0].values.length;
							}
						);
						v
							? U.attr("transform", function (a, c, d) {
									var e = 0;
									e = b[d].nonStackable
										? r(a, c) < 0
											? n(0)
									...
									var f = 0;
									b[d].nonStackable &&
										((f = (a.series * m.rangeBand()) / b.length),
										b.length !== I &&
											(f =
												(b[d].nonStackableSeries * m.rangeBand()) / (2 * I)));
									var g = f + m(q(a, c));
							...
							: U.attr("transform", function (a, c) {
									var d = (a.series * m.rangeBand()) / b.length + m(q(a, c)),
										e =
											r(a, c) < 0
												? n(0)
												: n(0) - n(r(a, c)) < 1
							...
							(h = m.copy()),
							(i = n.copy()),
							b[0] && b[0].values && (F = b[0].values.length);
					}),
					E.renderEnd("multibar immediate"),
					b
				);
			}
			var c,
						...
						var M = L.enter()
							...
							.attr("transform", function (a, c, d) {
								return (
									...
									i(x ? a.y0 : 0) +
									"," +
									(x ? 0 : (d * o.rangeBand()) / b.length + o(q(a, c))) +
									")"
								);
							});
						M.append("rect")
							.attr("width", 0)
							.attr("height", o.rangeBand() / (x ? 1 : b.length)),
							...
							s(b[0].values[0], 0) &&
								...
								L.select("polyline")
									...
									.attr("points", function (a, c) {
										var d = s(a, c),
											e = (0.8 * o.rangeBand()) / (2 * (x ? 1 : b.length));
										(d = d.length ? d : [-Math.abs(d), Math.abs(d)]),
											(d = d.map(function (a) {
												return p(a) - p(0);
											}));
				...
				return (
					j.each(function (j) {
						...
						function E(a) {
							...
							(a.value = v.x()(a.data)),
								(a.series = { value: v.y()(a.data), color: a.color }),
								D.duration(0)
									.valueFormatter(function (a, c) {
										return b.tickFormat()(a, c);
									})
						...
						a.utils.initSVG(F),
							...
							(b.container = this);
						var G = a.utils.availableWidth(g, F, e),
							H = a.utils.availableHeight(h, F, e),
						...
						o.domain(
							d3.extent(d3.merge(Q.concat(R)), function (a) {
								return a.x;
							})
						).range([0, G]);
						var S = F.selectAll("g.wrap.multiChart").data([j]),
							T = S.enter()
								.append("g")
								.attr("class", "wrap nvd3 multiChart")
								.append("g");
						T.append("g").attr("class", "nv-x nv-axis"),
							T.append("g").attr("class", "nv-y1 nv-axis"),
							T.append("g").attr("class", "nv-y2 nv-axis"),
							T.append("g").attr("class", "lines1Wrap"),
							T.append("g").attr("class", "lines2Wrap"),
							T.append("g").attr("class", "scatters1Wrap"),
							T.append("g").attr("class", "scatters2Wrap"),
							T.append("g").attr("class", "bars1Wrap"),
							T.append("g").attr("class", "bars2Wrap"),
							T.append("g").attr("class", "stack1Wrap"),
							T.append("g").attr("class", "stack2Wrap"),
							T.append("g").attr("class", "legendWrap");
						var U = S.select("g"),
							V = j.map(function (a, b) {
						...
						var Y = U.select(".lines1Wrap").datum(
								I.filter(function (a) {
									return !a.disabled;
								})
							),
							Z = U.select(".scatters1Wrap").datum(
								K.filter(function (a) {
									return !a.disabled;
								})
							),
							$ = U.select(".bars1Wrap").datum(
								M.filter(function (a) {
									return !a.disabled;
								})
							),
							_ = U.select(".stack1Wrap").datum(
								O.filter(function (a) {
									return !a.disabled;
								})
							),
							ab = U.select(".lines2Wrap").datum(
								J.filter(function (a) {
									return !a.disabled;
								})
							),
							bb = U.select(".scatters2Wrap").datum(
								L.filter(function (a) {
									return !a.disabled;
								})
							),
							cb = U.select(".bars2Wrap").datum(
								N.filter(function (a) {
									return !a.disabled;
								})
							),
							db = U.select(".stack2Wrap").datum(
								P.filter(function (a) {
									return !a.disabled;
								})
							),
						...
						p
							.domain(
								c ||
									d3.extent(d3.merge(Q).concat(eb), function (a) {
										return a.y;
									})
							)
							.range([0, H]),
							q
								.domain(
									d ||
										d3.extent(d3.merge(R).concat(fb), function (a) {
											return a.y;
										})
								)
								.range([0, H]),
							...
							w.yDomain(q.domain()),
							y.yDomain(q.domain()),
							O.length && d3.transition(_).call(x),
							P.length && d3.transition(db).call(y),
							M.length && d3.transition($).call(v),
							N.length && d3.transition(cb).call(w),
							I.length && d3.transition(Y).call(r),
							J.length && d3.transition(ab).call(s),
							K.length && d3.transition(Z).call(t),
							L.length && d3.transition(bb).call(u),
							z._ticks(a.utils.calcTicksX(G / 100, j)).tickSize(-H, 0),
							U.select(".nv-x.nv-axis").attr(
								"transform",
								"translate(0," + H + ")"
							),
							d3.transition(U.select(".nv-x.nv-axis")).call(z),
							A._ticks(a.utils.calcTicksY(H / 36, j)).tickSize(-G, 0),
							d3.transition(U.select(".nv-y1.nv-axis")).call(A),
							B._ticks(a.utils.calcTicksY(H / 36, j)).tickSize(-G, 0),
							d3.transition(U.select(".nv-y2.nv-axis")).call(B),
							U.select(".nv-y1.nv-axis")
								.classed("nv-disabled", Q.length ? !1 : !0)
								.attr("transform", "translate(" + o.range()[0] + ",0)"),
							U.select(".nv-y2.nv-axis")
								.classed("nv-disabled", R.length ? !1 : !0)
								.attr("transform", "translate(" + o.range()[1] + ",0)"),
							C.dispatch.on("stateChange", function () {
								b.update();
							}),
							...
							t.dispatch.on("elementMouseover.tooltip", l),
							u.dispatch.on("elementMouseover.tooltip", l),
							...
							x.dispatch.on("elementMouseover.tooltip", n),
							y.dispatch.on("elementMouseover.tooltip", n),
							x.dispatch.on("elementMouseout.tooltip", function () {
								D.hidden(!0);
							}),
							y.dispatch.on("elementMouseout.tooltip", function () {
								D.hidden(!0);
							}),
							v.dispatch.on("elementMouseover.tooltip", E),
							w.dispatch.on("elementMouseover.tooltip", E),
