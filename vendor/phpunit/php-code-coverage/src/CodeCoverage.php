...
class PHP_CodeCoverage
{
	...
	public function __construct(
		PHP_CodeCoverage_Driver $driver = null,
			...
			if ($this->processUncoveredFilesFromWhitelist) {
				...
				for ($i = 1; $i <= $lines; $i++) {
					$data[$uncoveredFile][$i] =
						PHP_CodeCoverage_Driver::LINE_NOT_EXECUTED;
				}
			}
		}
	 ...
	 */
	private function processUncoveredFileFromWhitelist(
		$uncoveredFile,
		array &$data,
		...
		foreach ($coverage as $file => $fileCoverage) {
			if (!isset($data[$file]) && in_array($file, $uncoveredFiles)) {
				foreach (array_keys($fileCoverage) as $key) {
					if (
						$fileCoverage[$key] ==
						PHP_CodeCoverage_Driver::LINE_EXECUTED
					) {
						$fileCoverage[$key] =
							PHP_CodeCoverage_Driver::LINE_NOT_EXECUTED;
					}
			...
			foreach ($tokens as $token) {
				switch (get_class($token)) {
					...
					case "PHP_Token_FUNCTION":
						...
						if (
							strpos($docblock, "@codeCoverageIgnore") ||
							...
							if (empty($classes[$token->getName()]["methods"])) {
								...
								do {
									...
									$lastMethod !== null &&
									substr($lastMethod["signature"], 0, 18) ==
										"anonymous function"
								);
	...
	private function performUnexecutedCoveredCodeCheck(
		...
		if (!empty($message)) {
			throw new PHP_CodeCoverage_CoveredCodeNotExecutedException(
				$message
			);
		}
	}
