...
EOF
	...
	public function testConstraintGreaterThanOrEqual()
	{
		...
		$this->assertFalse($constraint->evaluate(0, "", true));
		$this->assertEquals(
			"is equal to 1 or is greater than 1",
			$constraint->toString()
		);
...
EOF
	...
	public function testConstraintNotGreaterThanOrEqual()
	{
		...
		$this->assertEquals(
			"not( is equal to 1 or is greater than 1 )",
			$constraint->toString()
		);
		$this->assertEquals(2, count($constraint));
...
EOF
	...
	public function testConstraintIsInstanceOf()
	{
		...
		$interfaceConstraint = PHPUnit_Framework_Assert::isInstanceOf(
			"Countable"
		);
		$this->assertFalse(
			$interfaceConstraint->evaluate(new stdClass(), "", true)
		);
		$this->assertTrue(
			$interfaceConstraint->evaluate(new ArrayObject(), "", true)
		);
		$this->assertEquals(
			'is instance of interface "Countable"',
...
EOF
	 ...
	 * @covers PHPUnit_Framework_Assert::isType
	 */
	public function testConstraintIsResourceTypeEvaluatesCorrectlyWithResources(
		$resource
	) {
...
EOF
	...
	public function testConstraintLessThanOrEqual()
	{
		...
		$this->assertEquals(
			"is equal to 1 or is less than 1",
			$constraint->toString()
		);
		$this->assertEquals(2, count($constraint));
...
EOF
	...
	public function testConstraintCallback()
	{
		...
		$constraint = PHPUnit_Framework_Assert::callback(
...
EOF
	...
	public function testConstraintNotLessThanOrEqual()
	{
		...
		$this->assertFalse($constraint->evaluate(1, "", true));
		$this->assertEquals(
			"not( is equal to 1 or is less than 1 )",
			$constraint->toString()
		);
		$this->assertEquals(2, count($constraint));
...
EOF
	...
	{
		$constraint = PHPUnit_Framework_Assert::classHasAttribute(
			"privateAttribute"
		);

		$this->assertTrue(
			$constraint->evaluate("ClassWithNonPublicAttributes", "", true)
		);
		$this->assertFalse($constraint->evaluate("stdClass", "", true));
		$this->assertEquals(
			'has attribute "privateAttribute"',
			$constraint->toString()
		);
		$this->assertEquals(1, count($constraint));
...
EOF
	...
	public function testConstraintClassNotHasAttribute()
	{
		...
		$this->assertEquals(
			'does not have attribute "privateAttribute"',
			$constraint->toString()
		);
		$this->assertEquals(1, count($constraint));
...
EOF
	...
	{
		$constraint = PHPUnit_Framework_Assert::classHasStaticAttribute(
		...
		$this->assertFalse($constraint->evaluate("stdClass", "", true));
		$this->assertEquals(
			'has static attribute "privateStaticAttribute"',
			$constraint->toString()
		);
		$this->assertEquals(1, count($constraint));
...
EOF
	...
	public function testConstraintClassNotHasStaticAttribute()
	{
		...
		$this->assertEquals(
			'does not have static attribute "privateStaticAttribute"',
			$constraint->toString()
		);
		$this->assertEquals(1, count($constraint));
...
EOF
	...
	{
		$constraint = PHPUnit_Framework_Assert::objectHasAttribute(
			"privateAttribute"
		);

		$this->assertTrue(
			$constraint->evaluate(new ClassWithNonPublicAttributes(), "", true)
		);
		$this->assertFalse($constraint->evaluate(new stdClass(), "", true));
		$this->assertEquals(
			'has attribute "privateAttribute"',
...
EOF
	...
	{
		$constraint = PHPUnit_Framework_Assert::matchesRegularExpression(
			"/foo/"
		);
...
EOF
	...
	public function testConstraintStringMatches5()
	{
		...
		$this->assertEquals(
			'matches PCRE pattern "/^\*[0-9a-fA-F]+\*$/s"',
			$constraint->toString()
		);
		$this->assertEquals(1, count($constraint));
	}
	...
	public function testConstraintStringMatches6()
	{
		...
		$this->assertEquals(
			'matches PCRE pattern "/^\*[+-]?\.?\d+\.?\d*(?:[Ee][+-]?\d+)?\*$/s"',
			$constraint->toString()
		);
		$this->assertEquals(1, count($constraint));
	}
...
EOF
	...
	public function testAttributeNotEqualTo()
	{
		...
		$this->assertTrue($constraint->evaluate($object, "", true));
		$this->assertEquals(
			'attribute "foo" is not equal to 2',
			$constraint->toString()
		);
		$this->assertEquals(1, count($constraint));
...
EOF
	...
	public function testConstraintIsEmpty()
	{
		...
		$this->assertTrue($constraint->evaluate([], "", true));
		$this->assertFalse(
			$constraint->evaluate(new ArrayObject(["foo"]), "", true)
		);
...
EOF
	...
	public function testConstraintCountWithAnIteratorWhichDoesNotImplementCountable()
	{
		...
		$this->assertTrue(
			$constraint->evaluate(new TestIterator([1, 2, 3, 4, 5]), "", true)
		);
		$this->assertFalse(
			$constraint->evaluate(new TestIterator([1, 2, 3, 4]), "", true)
		);
	}
	...
	public function testConstraintCountWithAnObjectImplementingCountable()
	{
		...
		$this->assertTrue(
			$constraint->evaluate(new ArrayObject([1, 2, 3, 4, 5]), "", true)
		);
		$this->assertFalse(
			$constraint->evaluate(new ArrayObject([1, 2, 3, 4]), "", true)
		);
	}
...
EOF
	...
	{
		$constraint = new PHPUnit_Framework_Constraint_SameSize(
			new TestIterator([1, 2, 3, 4, 5])
		);

		$this->assertTrue(
			$constraint->evaluate(new TestIterator([6, 7, 8, 9, 10]), "", true)
		);
		$this->assertFalse(
			$constraint->evaluate(new TestIterator([1, 2, 3, 4]), "", true)
		);
	...
	public function testConstraintSameSizeWithAnObjectImplementingCountable()
	{
		...
		$this->assertTrue(
			$constraint->evaluate(new ArrayObject([6, 7, 8, 9, 10]), "", true)
		);
		$this->assertFalse(
			$constraint->evaluate(new ArrayObject([1, 2, 3, 4]), "", true)
		);
...
EOF
	...
	{
		$constraint = new PHPUnit_Framework_Constraint_Exception(
			"FoobarException"
		);
