...
class Util_TestTest extends PHPUnit_Framework_TestCase
{
	...
	public function testGetExpectedException()
	{
		...
		$this->assertArraySubset(
			[
				"class" => "Class",
				"code" =>
					"My\Space\ExceptionNamespaceTest::UNKNOWN_CODE_CONSTANT",
				"message" =>
		...
		return [
			...
			[
				...
				[
					...
					"PHPUnit" => "9-dev",
					"OS" => "/DOESNOTEXIST/i",
					"functions" => ["testFuncOne", "testFuncTwo"],
					"extensions" => [
						"testExtOne",
						"testExtTwo",
						"testExtThree",
	...
	public function testGetRequirementsMergesClassAndMethodDocBlocks()
	{
		$expectedAnnotations = [
			...
			"PHPUnit" => "3.7",
			"OS" => "/WINNT/i",
			"functions" => ["testFuncClass", "testFuncMethod"],
			"extensions" => ["testExtClass", "testExtMethod"],
		];
	...
	public function testGetProvidedDataRegEx()
	{
		...
		$result = preg_match(
			PHPUnit_Util_Test::REGEX_DATA_PROVIDER,
			"@dataProvider namespace\class::method",
			$matches
		);
		$this->assertEquals(1, $result);
	...
	public function testParseAnnotationThatIsOnlyOneLine()
	{
		$this->assertEquals(
			...
			PHPUnit_Util_Test::getDependencies(
				get_class($this),
				"methodForTestParseAnnotationThatIsOnlyOneLine"
			)
		);
	}
	...
	public function testGetLinesToBeCovered($test, $lines)
	{
		...
		$this->assertEquals(
			$expected,
			PHPUnit_Util_Test::getLinesToBeCovered($test, "testSomething")
		);
	}
	...
	public function getLinesToBeCoveredProvider()
	{
		return [
			...
			["CoverageClassTest", range(19, 36)],
			["CoverageMethodTest", range(31, 35)],
			["CoverageMethodOneLineAnnotationTest", range(31, 35)],
			[
			...
			["CoveragePrivateTest", range(21, 23)],
			["CoverageProtectedTest", range(25, 29)],
			["CoveragePublicTest", range(31, 35)],
			["CoverageFunctionTest", range(2, 4)],
			[
				"NamespaceCoverageClassExtendedTest",
				array_merge(range(21, 38), range(4, 19)),
			],
			["NamespaceCoverageClassTest", range(21, 38)],
			["NamespaceCoverageMethodTest", range(33, 37)],
			[
			...
			["NamespaceCoveragePrivateTest", range(23, 25)],
			["NamespaceCoverageProtectedTest", range(27, 31)],
			["NamespaceCoveragePublicTest", range(33, 37)],
			[
			...
			["NamespaceCoverageCoversClassPublicTest", range(33, 37)],
