...
class PHPUnit_Util_Configuration
{
	...
	protected function __construct($filename)
	{
		...
		$this->document = PHPUnit_Util_XML::loadFile(
			$filename,
			false,
			true,
			true
		...
		foreach ($this->xpath->query("listeners/listener") as $listener) {
			...
			foreach ($listener->childNodes as $node) {
				if (
					...
					$node->tagName == "arguments"
				) {
					foreach ($node->childNodes as $argument) {
						if ($argument instanceof DOMElement) {
							if (
								$argument->tagName == "file" ||
								...
								$arguments[] = PHPUnit_Util_XML::xmlToVariable(
									$argument
								);
							}
						}
	...
	protected function getTestSuite(
		...
		foreach (
			$testSuiteNode->getElementsByTagName("exclude")
			as $excludeNode
		) {
		...
		foreach (
			$testSuiteNode->getElementsByTagName("directory")
			as $directoryNode
		) {
			if (
				$testSuiteFilter &&
			...
			if ($directoryNode->hasAttribute("phpVersion")) {
				$phpVersion = (string) $directoryNode->getAttribute(
					"phpVersion"
				);
			} else {
			...
			if ($directoryNode->hasAttribute("phpVersionOperator")) {
				$phpVersionOperator = (string) $directoryNode->getAttribute(
					"phpVersionOperator"
				);
			} else {
				$phpVersionOperator = ">=";
		...
		foreach ($testSuiteNode->getElementsByTagName("file") as $fileNode) {
			...
			if ($fileNode->hasAttribute("phpVersionOperator")) {
				$phpVersionOperator = (string) $fileNode->getAttribute(
					"phpVersionOperator"
				);
			} else {
				$phpVersionOperator = ">=";
