 ...
 */
abstract class PHPUnit_Util_TestDox_ResultPrinter
	...
	implements PHPUnit_Framework_TestListener
{
	...
	public function startTest(PHPUnit_Framework_Test $test)
	{
		...
		if ($this->testClass != $class) {
			...
			$classAnnotations = PHPUnit_Util_Test::parseTestMethodAnnotations(
				$class
			);
			if (isset($classAnnotations["class"]["testdox"][0])) {
				$this->currentTestClassPrettified =
					$classAnnotations["class"]["testdox"][0];
			} else {
				$this->currentTestClassPrettified = $this->prettifier->prettifyTestClass(
					$class
				);
			}
		...
		if (isset($annotations["method"]["testdox"][0])) {
			$this->currentTestMethodPrettified =
				$annotations["method"]["testdox"][0];
		} else {
			$this->currentTestMethodPrettified = $this->prettifier->prettifyTestMethod(
				$test->getName(false)
			);
		}
	...
	private function isOfInterest(PHPUnit_Framework_Test $test)
	{
		return $test instanceof PHPUnit_Framework_TestCase &&
			get_class($test) != "PHPUnit_Framework_WarningTestCase";
	}
}
