...
class PHPUnit_Util_Test
{
	...
	public static function getLinesToBeCovered($className, $methodName)
	{
		...
		return self::getLinesToBeCoveredOrUsed(
			$className,
			$methodName,
			"covers"
		);
	 ...
	 */
	private static function getLinesToBeCoveredOrUsed(
		$className,
		$methodName,
		$mode
	) {
	...
	public static function getMissingRequirements($className, $methodName)
	{
		...
		if (
			!empty($required["PHP"]) &&
			version_compare(PHP_VERSION, $required["PHP"], "<")
		) {
			$missing[] = sprintf(
		...
		if (!empty($required["functions"])) {
			foreach ($required["functions"] as $function) {
				...
				if (
					2 === count($pieces) &&
					method_exists($pieces[0], $pieces[1])
				) {
					continue;
				}
		...
		if (!empty($required["extension_versions"])) {
			foreach (
				...
				as $extension => $minVersion
			) {
				$actualVersion = phpversion($extension);
				if (
					false === $actualVersion ||
					version_compare($actualVersion, $minVersion, "<")
				) {
					$missing[] = sprintf(
						"Extension %s %s (or later) is required.",
						$extension,
						$minVersion
					);
				}
	...
	public static function getExpectedException($className, $methodName)
	{
		...
		if (preg_match(self::REGEX_EXPECTED_EXCEPTION, $docComment, $matches)) {
			...
			if (isset($matches[2])) {
				...
				isset($annotations["method"]["expectedExceptionMessage"])
			) {
			...
			if (
				isset($annotations["method"]["expectedExceptionMessageRegExp"])
			) {
	...
	public static function getProvidedData($className, $methodName)
	{
		...
		if (
			$dataProviderData = self::getDataFromDataProviderAnnotation(
				$docComment,
				$className,
				$methodName
			)
	...
	public static function parseTestMethodAnnotations(
		...
		if (
			!empty($methodName) &&
			!isset(self::$annotationCache[$className . "::" . $methodName])
		) {
			try {
		...
		return [
			...
			"method" => !empty($methodName)
				? self::$annotationCache[$className . "::" . $methodName]
				: [],
		];
	}
	...
	public static function getProcessIsolationSettings($className, $methodName)
	{
		...
		if (
			isset($annotations["class"]["runTestsInSeparateProcesses"]) ||
	 ...
	 * @since  Method available since Release 3.4.0
	 */
	public static function getPreserveGlobalStateSettings(
		$className,
		$methodName
	) {
		return self::getBooleanAnnotationSetting(
	 ...
	 * @since  Method available since Release 3.4.0
	 */
	private static function getBooleanAnnotationSetting(
		$className,
		$methodName,
		$settingName
	) {
	...
	private static function isBeforeClassMethod(ReflectionMethod $method)
	{
		return $method->isStatic() &&
			strpos($method->getDocComment(), "@beforeClass") !== false;
	}
