	...
	IteratorAggregate
{
	...
	public function __construct($theClass = "", $name = "")
	{
		...
		if (!$theClass->isSubclassOf("PHPUnit_Framework_TestCase")) {
			throw new PHPUnit_Framework_Exception(
				'Class "' .
					$theClass->name .
					'" does not extend PHPUnit_Framework_TestCase.'
			);
		}
		...
		foreach ($newClasses as $className) {
			...
			if (!$class->isAbstract()) {
				if (
					...
					$class->implementsInterface("PHPUnit_Framework_Test")
				) {
		...
		if ($constructor !== null) {
			...
			// TestCase($name, $data)
				...
				if (isset($data)) {
					...
					if (
						$data instanceof PHPUnit_Framework_WarningTestCase ||
	...
	public function run(PHPUnit_Framework_TestResult $result = null)
	{
		...
		foreach ($this as $test) {
			...
			if (
				...
				$test->setRunTestInSeparateProcess(
	...
	{
		return new PHPUnit_Framework_SkippedTestCase(
			$class,
			$methodName,
			$message
		);
	...
	public function setbeStrictAboutChangesToGlobalState(
		...
		if (
			is_null($this->beStrictAboutChangesToGlobalState) &&
