		...
		PHPUnit_Framework_SelfDescribing
{
	...
	public function run(PHPUnit_Framework_TestResult $result = null)
	{
		...
		if (
			!$this instanceof PHPUnit_Framework_WarningTestCase &&
	...
	{
		if (
			is_null($this->backupStaticAttributes) &&
	 ...
	 */
	public function getMockWithoutInvokingTheOriginalConstructor(
		$originalClassName
	) {
			...
			foreach ($this->dependencies as $dependency) {
				...
				if (isset($passed[$dependency])) {
					...
					if ($clone) {
						...
						$this->dependencyInput[$dependency] = $deepCopy->copy(
							$passed[$dependency]["result"]
						);
					} else {
						$this->dependencyInput[$dependency] =
							$passed[$dependency]["result"];
					}
				} else {
	 ...
	 * @since  Method available since Release 5.0.0
	 */
	private function shouldInvocationMockerBeReset(
	...
	private function registerMockObjectsFromTestArguments(array $testArguments)
	{
		foreach ($testArguments as $testArgument) {
			if (
				$testArgument instanceof PHPUnit_Framework_MockObject_MockObject
			) {
