...
class PHPUnit_TextUI_Command
{
	...
	protected function handleArguments(array $argv)
	{
			...
			switch ($option[0]) {
				case "--colors":
					$this->arguments["colors"] =
						$option[1] ?: PHPUnit_TextUI_ResultPrinter::COLOR_AUTO;
					break;
		...
		if (
			...
			$configurationFile =
				$this->arguments["configuration"] . "/phpunit.xml";
		...
		if (isset($this->arguments["configuration"])) {
			 ...
			 */
			if (
				isset($phpunitConfiguration["stderr"]) &&
			...
			if (
				isset($phpunitConfiguration["columns"]) &&
				!isset($this->arguments["columns"])
			) {
			...
			if (
				...
				isset($phpunitConfiguration["printerClass"])
			) {
			...
			if (!isset($this->arguments["test"])) {
				$testSuite = $configuration->getTestSuiteConfiguration(
					isset($this->arguments["testsuite"])
		...
		if (
			!isset($this->arguments["test"]) ||
			(isset($this->arguments["testDatabaseLogRevision"]) &&
				!isset($this->arguments["testDatabaseDSN"]))
		) {
			$this->showHelp();
	...
	protected function handleLoader($loaderClass, $loaderFile = "")
	{
		...
		if (class_exists($loaderClass, false)) {
			...
			if (
				$class->implementsInterface("PHPUnit_Runner_TestSuiteLoader") &&
		...
		$this->showError(
			sprintf('Could not use "%s" as loader.', $loaderClass)
		);
	}
	...
	protected function handlePrinter($printerClass, $printerFile = "")
	{
		...
		if (class_exists($printerClass)) {
			...
			if (
				$class->implementsInterface("PHPUnit_Framework_TestListener") &&
				...
				$outputStream = isset($this->arguments["stderr"])
					? "php://stderr"
					: null;
		...
		$this->showError(
			sprintf('Could not use "%s" as printer.', $printerClass)
		);
	}
	...
	protected function handleVersionCheck()
	{
		...
		if ($isOutdated) {
			...
			print 'Use "phpunit --self-upgrade" to install PHPUnit ' .
				$latestVersion .
				"\n";
		} else {
