...
class vfsStreamWrapper
{
	...
	public function stream_open($path, $mode, $options, $opened_path)
	{
		...
		if (in_array($mode, ["r", "w", "a", "x", "c"]) === false) {
			if (($options & STREAM_REPORT_ERRORS) === STREAM_REPORT_ERRORS) {
				trigger_error(
					"Illegal mode " .
						$mode .
						", use r, w, a, x  or c, flavoured with b and/or +",
					E_USER_WARNING
				);
			}
		...
		$this->content = $this->getContentOfType(
			$path,
			vfsStreamContent::TYPE_FILE
		);
			...
			if (self::WRITE === $mode) {
				if (
					...
					trigger_error(
						"File " .
							$path .
							" already exists, can not open with mode x",
						E_USER_WARNING
					);
				}
	...
	private function createFile($path, $mode = null, $options = null)
	{
		...
		if (empty($names["dirname"]) === true) {
			if (($options & STREAM_REPORT_ERRORS) === STREAM_REPORT_ERRORS) {
				trigger_error(
					"File " . $names["basename"] . " does not exist",
					E_USER_WARNING
				);
			}
		...
		if (null === $dir) {
			if (($options & STREAM_REPORT_ERRORS) === STREAM_REPORT_ERRORS) {
				trigger_error(
					"Directory " . $names["dirname"] . " does not exist",
					E_USER_WARNING
				);
			}
			...
			if (($options & STREAM_REPORT_ERRORS) === STREAM_REPORT_ERRORS) {
				trigger_error(
					"Directory " .
						$names["dirname"] .
						" already contains a director named " .
						$names["basename"],
					E_USER_WARNING
				);
			}
		...
		if (
			...
			if (($options & STREAM_REPORT_ERRORS) === STREAM_REPORT_ERRORS) {
				trigger_error(
					"Can not create new file in non-writable path " .
						$names["dirname"],
					E_USER_WARNING
				);
			}
	...
	public function stream_write($data)
	{
		...
		if (self::$quota->isLimited()) {
			$data = substr(
				$data,
				0,
				self::$quota->spaceLeft(self::$root->sizeSummarized())
			);
		}
	...
	public function stream_stat()
	{
		$fileStat = [
			...
			"ino" => 0,
			"mode" =>
				$this->content->getType() | $this->content->getPermissions(),
			"nlink" => 0,
	...
	public function unlink($path)
	{
		...
		if ($content->getType() !== vfsStreamContent::TYPE_FILE) {
			trigger_error(
				"unlink(" . $path . "): Operation not permitted",
				E_USER_WARNING
			);
			return false;
		}
	...
	public function url_stat($path, $flags)
	{
		$content = $this->getContent(
			$this->resolvePath(vfsStream::path($path))
		);
		if (null === $content) {
