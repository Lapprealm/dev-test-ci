...
class vfsStreamWrapperQuotaTestCase extends \PHPUnit_Framework_TestCase
{
	...
	public function writeLessThanQuotaWritesEverything()
	{
		$this->assertEquals(
			9,
			file_put_contents(vfsStream::url("root/file.txt"), "123456789")
		);
		$this->assertEquals(
			"123456789",
	...
	public function writeUpToQotaWritesEverything()
	{
		$this->assertEquals(
			10,
			file_put_contents(vfsStream::url("root/file.txt"), "1234567890")
		);
		$this->assertEquals(
			"1234567890",
			$this->root->getChild("file.txt")->getContent()
	...
	public function writeMoreThanQotaWritesOnlyUpToQuota()
	{
		try {
			...
			$this->assertEquals(
				"file_put_contents(): Only 10 of 11 bytes written, possibly out of free disk space",
				$e->getMessage()
			);
		}
	...
	public function considersAllFilesForQuota()
	{
		...
		try {
			...
			$this->assertEquals(
				"file_put_contents(): Only 7 of 11 bytes written, possibly out of free disk space",
				$e->getMessage()
			);
		}
