...
class vfsStreamWrapperMkDirTestCase extends vfsStreamWrapperBaseTestCase
{
	...
	public function mkdirNonRecursively()
	{
		...
		$this->assertEquals(
			0777,
			$this->foo->getChild("another")->getPermissions()
		);
	}
	...
	{
		$this->assertTrue(
			mkdir($this->fooURL . "/another/../more/.", 0777, true)
		);
		$this->assertEquals(3, count($this->foo->getChildren()));
	...
	public function mkdirWithoutRootCreatesNewRoot()
	{
		...
		$this->assertEquals(
			...
			vfsStreamWrapper::getRoot()->getType()
		);
		$this->assertEquals("foo", vfsStreamWrapper::getRoot()->getName());
		$this->assertEquals(
			0777,
			vfsStreamWrapper::getRoot()->getPermissions()
		);
	}
	...
	public function mkdirDirCanNotCreateNewDirInNonWritingDirectory()
	{
		...
		vfsStreamWrapper::getRoot()->addChild(
			new vfsStreamDirectory("restrictedFolder", 0000)
		);
		$this->assertFalse(
			is_writable(vfsStream::url("root/restrictedFolder/"))
		);
		$this->assertFalse(
			mkdir(vfsStream::url("root/restrictedFolder/newFolder"))
		);
		$this->assertFalse(
			vfsStreamWrapper::getRoot()->hasChild("restrictedFolder/newFolder")
		);
	}
	...
	public function directoryIteration()
	{
		...
		$this->assertEquals(
			2,
			$i,
			"Directory foo contains two children, but got " .
				$i .
	...
	public function issue17()
	{
		...
		$this->assertFalse(
			vfsStreamWrapper::getRoot()->hasChild("doesNotWork")
		);
	}
	...
	public function unlinkCanNotRemoveNonEmptyDirectory()
	{
		try {
			...
			$this->assertEquals(
				"unlink(vfs://foo/bar): Operation not permitted",
				$fe->getMessage()
			);
		}
	...
	public function unlinkCanNotRemoveEmptyDirectory()
	{
		...
		try {
			...
			$this->assertEquals(
				"unlink(vfs://foo/empty): Operation not permitted",
				$fe->getMessage()
			);
		}
	...
	public function canCreateFolderOfSameNameAsParentFolder()
	{
		...
		mkdir(
			vfsStream::url("testFolder") . "/testFolder/subTestFolder",
			0777,
			true
		);
		$this->assertTrue(
			file_exists(vfsStream::url("testFolder/testFolder/subTestFolder/."))
		);
	}
