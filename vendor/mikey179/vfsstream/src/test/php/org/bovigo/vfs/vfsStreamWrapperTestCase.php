...
class vfsStreamWrapperTestCase extends vfsStreamWrapperBaseTestCase
{
	...
	public function unlinkRemovesFilesOnly()
	{
		...
		$this->assertEquals([$this->bar], $this->foo->getChildren());
	...
	public function basename()
	{
		...
		$this->assertEquals(
			"doesNotExist",
			basename(vfsStream::url("doesNotExist"))
		);
	}
	...
	public function fileownerIsCurrentUserByDefault()
	{
		...
		$this->assertEquals(
			vfsStream::getCurrentUser(),
			fileowner($this->baz1URL)
		);
		$this->assertEquals(
			vfsStream::getCurrentUser(),
			fileowner($this->baz2URL)
		);
	}
	...
	public function groupIsCurrentGroupByDefault()
	{
		...
		$this->assertEquals(
			vfsStream::getCurrentGroup(),
			filegroup($this->baz1URL)
		);
		$this->assertEquals(
			vfsStream::getCurrentGroup(),
			filegroup($this->baz2URL)
		);
	}
