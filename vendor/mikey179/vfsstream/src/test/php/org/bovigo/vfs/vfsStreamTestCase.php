...
class vfsStreamTestCase extends \PHPUnit_Framework_TestCase
{
	...
	public function path()
	{
		...
		$this->assertEquals(
			"foo/bar.baz",
			vfsStream::path("vfs://foo/bar.baz")
		);
		$this->assertEquals(
			"foo/bar.baz",
			vfsStream::path("vfs://foo\bar.baz")
	...
	{
		$root = vfsStream::setup();
		$this->assertSame($root, vfsStream::create(["test" => []]));
		$this->assertTrue($root->hasChildren());
		$this->assertTrue($root->hasChild("test"));
		$this->assertInstanceOf(
			'org\\bovigo\\vfs\\vfsStreamDirectory',
	...
	public function inspectWithContentGivesContentToVisitor()
	{
		...
		$this->assertSame(
			$mockVisitor,
			vfsStream::inspect($mockVisitor, $mockContent)
		);
	}
	...
	{
		return realpath(
			dirname(__FILE__) . "/../../../../resources/filesystemcopy"
		);
	}
	...
	public function copyFromEmptyFolder()
	{
		$baseDir = vfsStream::copyFromFileSystem(
			$this->getFileSystemCopyDir() . "/emptyFolder",
	...
	public function copyFromWithSubFolders()
	{
		...
		$this->assertTrue(
			$subfolderDir->getChild("subfolder1")->hasChild("file1.txt")
		);
		$this->assertVfsFile(
			$subfolderDir->getChild("subfolder1/file1.txt"),
			""
		);
	...
	public function copyFromPreservesFilePermissions()
	{
		...
		$this->assertEquals(
			$root,
			vfsStream::copyFromFileSystem($copyDir, null)
		);
		$this->assertEquals(
			fileperms($copyDir . "/withSubfolders") -
				vfsStreamContent::TYPE_DIR,
			$root->getChild("withSubfolders")->getPermissions()
		);
		$this->assertEquals(
			fileperms($copyDir . "/withSubfolders/aFile.txt") -
