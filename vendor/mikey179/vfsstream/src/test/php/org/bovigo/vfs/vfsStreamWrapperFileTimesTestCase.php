...
class vfsStreamWrapperFileTimesTestCase extends \PHPUnit_Framework_TestCase
{
	 ...
	 */
	protected function assertFileTimesEqualStreamTimes(
		$url,
	...
	public function createNewFileSetsAllTimesToCurrentTime()
	{
		...
		$this->assertFileTimesEqualStreamTimes(
			$this->fooUrl,
			vfsStreamWrapper::getRoot()->getChild("foo.txt")
		);
	}
	...
	public function renameFileChangesAttributeAndModificationTimeOfAffectedDirectories()
	{
		...
		$source = vfsStream::newDirectory("bar")->at(
		...
		$this->assertLessThanOrEqual(
			time(),
			filemtime(vfsStream::url("root/target"))
		);
		$this->assertLessThanOrEqual(
			time(),
			filectime(vfsStream::url("root/target"))
		);
	...
	public function renameFileDoesNotChangeFileTimesOfFileItself()
	{
		...
		$this->assertEquals(
			300,
			filemtime(vfsStream::url("root/target/baz.txt"))
		);
		$this->assertEquals(
			300,
			filectime(vfsStream::url("root/target/baz.txt"))
		);
		$this->assertEquals(
			300,
			fileatime(vfsStream::url("root/target/baz.txt"))
		);
		$this->assertFileTimesEqualStreamTimes(
			vfsStream::url("root/target/baz.txt"),
			$file
		);
	}
