...
class vfsStreamDirectoryTestCase extends \PHPUnit_Framework_TestCase
{
	...
	public function childHandling()
	{
		...
		$this->assertEquals([$mockChild], $this->dir->getChildren());
	...
	public function explicitTestForSeparatorWithNestedPaths_Bug_24()
	{
		...
		$this->assertTrue(
			$this->dir->hasChild("subdir1"),
			"Level 1 path with separator exists"
		);
		$this->assertTrue(
			$this->dir->hasChild("subdir1/subdir2"),
			"Level 2 path with separator exists"
		);
		$this->assertTrue(
			$this->dir->hasChild("subdir1/subdir2/bar"),
			"Level 3 path with separator exists"
		);
		$this->assertFalse(
			$this->dir->hasChild("subdir1.subdir2"),
		...
		$this->assertFalse(
			$this->dir->hasChild("subdir1.subdir2/bar"),
			"Nested path with period does not exist"
		);
	}
	...
	public function owner()
	{
		...
		$this->assertTrue(
			$this->dir->isOwnedByUser(vfsStream::getCurrentUser())
		);
		$this->assertSame(
			$this->dir,
	...
	public function group()
	{
		...
		$this->assertTrue(
			$this->dir->isOwnedByGroup(vfsStream::getCurrentGroup())
		);
		$this->assertSame(
			$this->dir,
