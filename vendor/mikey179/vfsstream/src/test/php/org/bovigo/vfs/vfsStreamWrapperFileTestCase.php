...
class vfsStreamWrapperFileTestCase extends vfsStreamWrapperBaseTestCase
{
	...
	public function file_get_contentsNonReadableFile()
	{
		...
		vfsStream::newFile("new.txt", 0000)
			->at(vfsStreamWrapper::getRoot())
			->withContent("content");
		$this->assertEquals(
			"",
			@file_get_contents(vfsStream::url("root/new.txt"))
		);
	}
	...
	public function file_put_contentsExistingFile()
	{
		$this->assertEquals(
			14,
			file_put_contents($this->baz2URL, "baz is not bar")
		);
		...
		$this->assertFalse(
			@file_put_contents($this->barURL, "This does not work.")
		);
		$this->assertFalse(
			@file_put_contents($this->fooURL, "This does not work, too.")
		);
	}
	...
	public function file_put_contentsNonExistingFile()
	{
		$this->assertEquals(
			14,
			file_put_contents($this->fooURL . "/baznot.bar", "baz is not bar")
		);
		$this->assertEquals(3, count($this->foo->getChildren()));
		$this->assertEquals(
			14,
			file_put_contents($this->barURL . "/baznot.bar", "baz is not bar")
		);
	...
	{
		$this->assertFalse(
			@fopen(vfsStream::url("foo/doesNotExist.txt"), "rb")
		);
	}
	...
	public function cannotRenameToNonWritableDir()
	{
		$this->bar->chmod(0);
		$this->assertFalse(
			@rename($this->baz2URL, vfsStream::url("foo/bar/baz3"))
		);
	}
